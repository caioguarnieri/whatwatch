...
const FetchMovie = ({ setFindMovie }) => {
  ...
  useEffect(() => {
    ...
    const rollNumber = () => {
      ...
      if (filters.visitedPageResults.includes(randomNumber))
        return rollPageNumber();
      else {
        return randomNumber;
      }
    };
    ...
    const mainFunction = () => {
      ...
      if (storedFilters === null) storedFilters = filters;
        ...
        storedGenres = storedGenres.filter((item) => {
          return item.active === true;
        });
        storedGenres = storedGenres.map((item) => {
          return item.id;
        });
      ...
      if (filters.visitedPages === "") {
        axios
          .get(
            `https://api.themoviedb.org/3/discover/movie?api_key=${APIKey}&include_video=false&primary_release_date.gte=${
              storedFilters.releasedFrom
            }-01-01&primary_release_date.lte=${
              storedFilters.releasedTo
            }-12-31&vote_count.gte=${Number(
              storedFilters.votesOver
            )}&vote_average.gte=${Number(
              storedFilters.ratedOver
            )}&with_genres=${storedGenres}`
          )
          .then((res) => {
            ...
            if (res.data.total_results === 0) {
              ...
              return axios.get(
                `https://api.themoviedb.org/3/discover/movie?api_key=${APIKey}&include_video=false&primary_release_date.gte=${
                  ...
                  storedFilters.ratedOver
                )}&with_genres=${storedGenres}&page=${randomPage}`
              );
            }
          })
          .then((res) => {
            if (res === "No Movies")
              alert("No movies were found with tese filters");
            else {
              ...
              setFilters({
                ...
                visitedPages: [res.data.page],
                totalPageResults: totalMovies,
                visistedPageResults: [randomMovie],
                visitedResults: 1,
              });
        ...
        if (filters.totalResults === filters.visitedResults) {
          ...
          filters.totalPageResults === filters.visistedPageResults.length
        ) {
          ...
          axios
            ...
            .then((res) => {
              ...
              setFilters({
                ...
                visitedPageResults: [randomMovie],
                visitedResults: filters.visitedResults + 1,
              });
              history.push(`/movie/${res.data.results[randomMovie].id}`);
            });
        } else {
          axios
            .get(
              `https://api.themoviedb.org/3/discover/movie?api_key=${APIKey}&include_video=false&primary_release_date.gte=${
                ...
                storedFilters.ratedOver
              )}&with_genres=${storedGenres}&page=${
                filters.visitedPages[filters.visitedPages.length - 1]
              }`
            )
            .then((res) => {
              ...
              setFilters({
                ...filters,
                visitedPageResults: visitedPageResult,
                visitedResults: filters.visitedResults + 1,
              });
